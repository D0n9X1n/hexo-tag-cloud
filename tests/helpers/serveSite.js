 ...
 * @property {() => Promise<void>} close - shuts the server down.
 */

/**
 * Start a minimal static HTTP server bound to `127.0.0.1` on an ephemeral
 * port. Tests-only — no caching, range requests, or compression.
 *
 * Security: any path containing `..` (after URL decoding + normalization)
 * is rejected with 400. The resolved file path is verified to live under
 * `root` before being served. Missing files return 404.
 *
