...
const { computeApplyAction, renderUnifiedDiff } = require('./apply-edit');
...
async function runCli(opts) {
  ...
  if (sub === 'install-skill') {
    const { runInstallSkill } = require('./install-skill');
    return runInstallSkill(subArgs, { cwd, stdout, stderr, fs: fsImpl });
  }
  // Unknown subcommand
...
async function runInstall(argv, ctx) {
  ...
  const action = !fileExisted
    ? {
      kind: 'insert',
      newContent: newBlock,
      diff: renderUnifiedDiff('before', 'after', '', newBlock.split('\r\n').join('\n')),
    }
    : computeApplyAction({
      existingContent,
      newBlock,
